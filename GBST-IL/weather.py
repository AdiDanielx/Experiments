...
# Run AXGB comparison for each unique combination
axgb_results = []
...
for i, combo in enumerate(unique_combos):
    ...
    print(f"\n{'='*80}")
    print(f"AXGB Combo {i+1}/{len(unique_combos)}: {combo['name']}")
    print(f"Extended features: {ext_feats}")
    print(f"Basic features count: {len(base_feats)}")
    print(f"{'='*80}")
    sys.stdout.flush()
    ...
    axgb_results.append({
        'combo': combo['name'],
        'auc_without_extended': result['auc_without_extended'],
        'auc_with_all': result['auc_with_all'],
...
# --- AXGB Final Summary ---
print(f"\n{'='*120}")
print("AXGB FINAL SUMMARY")
print(f"{'='*120}")
print(f"{'Combo':<45} {'AUC (Basic)':<15} {'AUC (All)':<15} {'Difference':<15}")
print("-" * 120)
for r in axgb_results:
    print(f"{r['combo']:<45} {r['auc_without_extended']:<15.4f} {r['auc_with_all']:<15.4f} {r['difference']:<+15.4f}")

# Count improvements
improvements = [r for r in axgb_results if r['difference'] > 0]
print(f"\nAXGB Improvements (positive difference): {len(improvements)}/{len(axgb_results)}")
