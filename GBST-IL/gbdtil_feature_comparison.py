"""
GBDT-IL Feature Comparison Class

This class runs GBDT-IL twice:
...
try:
    from GBDTIL import GBDTILClassifier
except ImportError:
    # Fallback for direct execution
    import importlib.util
    spec = importlib.util.spec_from_file_location("GBDTIL", os.path.join(os.path.dirname(__file__), "GBDT-IL.py"))
    gbdtil_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(gbdtil_module)
    GBDTILClassifier = gbdtil_module.GBDTILClassifier


class GBDTILFeatureComparison:
    """
    Compare GBDT-IL performance with and without extended features.
    ...
    n_estimators : int, default=100
        Number of boosting iterations for GBDT-IL
    learning_rate : float, default=0.1
        Learning rate for GBDT-IL
    max_depth : int, default=3
        Maximum depth for GBDT-IL trees
    max_window_size : int, default=50
        Maximum window size for GBDT-IL
    subsample : float, default=1.0
        Subsample ratio for GBDT-IL
    ...
    def _train_and_evaluate(self, features, run_name):
        """
        Train GBDT-IL model and evaluate AUC.
    ...
    def run(self):
        """
        Run GBDT-IL twice and print AUC scores.
        ...
        print("\n[1] Running GBDT-IL WITHOUT extended features...")
        ...
        print("\n[2] Running GBDT-IL WITH all features (including extended)...")
